...
const RuleDetailsButtons = ({ rule, rulesSource }: Props) => {
  ...
  if (isCloudRulesSource(rulesSource) && exploreSupported && exploreAllowed && !isFederated) {
    buttons.push(
      ...
      >
        <Trans i18nKey="rule-details-buttons.view-graph">See graph</Trans>
      </LinkButton>
    );
  }
  if (rule.annotations[Annotation.runbookURL]) {
    buttons.push(
      ...
      >
        <Trans i18nKey="rule-details-buttons.view-runbook">View runbook</Trans>
      </LinkButton>
    );
  }
  if (rule.annotations[Annotation.dashboardUID]) {
    ...
    if (dashboardUID) {
      buttons.push(
        ...
        >
          <Trans i18nKey="rule-details-buttons.go-to-dashboard">Go to dashboard</Trans>
        </LinkButton>
      );
      ...
      if (panelId) {
        buttons.push(
          ...
          >
            <Trans i18nKey="rule-details-buttons.go-to-panel">Go to panel</Trans>
          </LinkButton>
        );
      }
    }
  ...
  if (isGrafanaRulerRule(rule.rulerRule)) {
    buttons.push(
      <Fragment key="history">
        ...
        >
          <Trans i18nKey="rule-details-buttons.show-state-history">Show state history</Trans>
        </Button>
        {StateHistoryModal}
      </Fragment>
    );
